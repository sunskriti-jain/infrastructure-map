  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 z-10 px-4 pt-3 flex items-center gap-3">
        {/* Logo */}
        <div className="flex items-center gap-2 bg-[#0a0f1e]/80 border border-[#1e293b] rounded-xl px-3 py-2 backdrop-blur-sm shrink-0">
          <span className="text-blue-400 text-lg">⚡</span>
          <span className="text-white font-bold text-sm hidden sm:block">US Grid Atlas</span>
        </div>

        {/* Search */}
        <div className="relative flex-1 max-w-lg mx-auto">
          <div className="flex items-center bg-[#0f172a]/90 border border-[#1e293b] rounded-xl px-3 py-2 backdrop-blur-sm focus-within:border-blue-500 transition-colors">
          ...
          {searchResults.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-[#0f172a] border border-[#1e293b] rounded-xl overflow-hidden shadow-xl z-50">
              {searchResults.map((p) => (
                <button
                  key={p.id}
                  onClick={() => handleSelectResult(p)}
                  className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-[#1e293b] text-left transition-colors"
                >
                  <span
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-slate-400 text-xs">{p.state} · {FUEL_LABELS[p.fuel]}</p>
                  </div>
                  {p.capacityMW > 0 && (
        ...
        {/* Stats pill */}
        <div className="hidden md:flex items-center gap-3 bg-[#0a0f1e]/80 border border-[#1e293b] rounded-xl px-3 py-2 backdrop-blur-sm shrink-0">
      ...
      {/* Legend strip */}
      <div className="absolute top-16 left-1/2 -translate-x-1/2 z-10 flex gap-2 bg-[#0a0f1e]/80 border border-[#1e293b] rounded-full px-3 py-1.5 backdrop-blur-sm flex-wrap justify-center max-w-[90vw]">
        {[...ALL_FUELS].map((fuel) => (
          <button
            key={fuel}
          ...
          >
            <span
              ...
              style={{ background: activeFuels.has(fuel) ? FUEL_COLORS[fuel] : "#334155" }}
            />
            {FUEL_LABELS[fuel]}
          </button>
        ))}
